  ...
  shimmer.wrap(Router.prototype, 'match', function (orig) {
    return function (_, method) {
      ...
      if (matched && matched.pathAndMethod && matched.pathAndMethod.length) {
        var match =
          matched.pathAndMethod.reduce((matchedLayer, layer) => {
            if (matchedLayer) return matchedLayer

            const hasMiddlewareWithoutNext = layer.stack.reduce(
              (hasFinalMiddleware, middleware) => {
                if (hasFinalMiddleware) return hasFinalMiddleware
                return middleware.length <= 1
              },
              false,
            )

            return hasMiddlewareWithoutNext ? layer : null
          }, null) || matched.pathAndMethod[matched.pathAndMethod.length - 1]
